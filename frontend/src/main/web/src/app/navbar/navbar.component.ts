...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  onSearchUsers(event) {
    ...
    this.subscriptions.push(this.accountService.searchUsers(username).subscribe(
      (response: any) => {
        this.searchedUser = response['content'];
      },
      error => {
        console.log(error);
    ...
    this.subscriptions.push(
      this.postService.save(post).subscribe(
        (response: Post) => {
          ...
          this.savePicture(this.postPicture, postId);
          this.loadingService.isLoading.next(false);
  ...
  savePicture(picture: File, postId: number): void {
    this.subscriptions.push(
      this.postService.uploadPostPicture(picture, postId).subscribe(
        response => {
          if (response.type === HttpEventType.UploadProgress) {
            this.progress = (response.loaded / response.total) * 100;
          } else {
            console.log(response);
            this.OnNewPostSuccess(3);
          }
        },
        error => {
          console.log(error);
